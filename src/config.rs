...
impl Config {
    ...
    pub fn load() -> Result<Self, Error> {
        ...
        if changed { confy::store("boxes", None, config.clone())?; }
        Ok(config)
    }
    ...
    pub fn add_server(&mut self, server: &Server) {
        ...
        if self.servers.iter().any(|s| s.location == server.location) {
            ...
            self.servers.remove(self.servers.iter().position(|s| s.location == server.location).ok_or(Error::ResourceNotFound("Server not found".to_string())).unwrap());
    ...
    pub fn save_server(&mut self, server: &Server) -> Result<(), Error> {
        ...
        let index = self.servers.iter().position(|s| s.server_name == server.server_name).ok_or(Error::ResourceNotFound("Server not found".to_string()))?;
